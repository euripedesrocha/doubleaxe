 ...
 * directly Copyright (c) 2017 Two Blue Cubes Ltd. All rights reserved.
 *
...
#if !defined(CATCH_INTERNAL_CONFIG_NO_POSIX_SIGNALS) && \
...
struct AutoReg : NonCopyable {
  AutoReg(ITestInvoker* invoker, SourceLineInfo const& lineInfo,
          StringRef classOrMethod, NameAndTags const& nameAndTags) noexcept;
  ~AutoReg();
};
...
#pragma warning(disable : 4180)  // We attempt to stream a function (address) by
                                 // const&, which MSVC complains about but is
                                 // harmless
#endif
...
struct StringMaker<std::pair<T1, T2>> {
  static std::string convert(const std::pair<T1, T2>& pair) {
    std::ostringstream oss;
    oss << "{ " << ::Catch::Detail::stringify(pair.first) << ", "
...
class BinaryExpr : public ITransientExpression {
 ...
 public:
  BinaryExpr(bool comparisonResult, LhsT lhs, StringRef op, RhsT rhs)
      : m_result(comparisonResult), m_lhs(lhs), m_op(op), m_rhs(rhs) {}
};
...
class ExprLhs {
 ...
 public:
  ...
  auto operator==(RhsT const& rhs) -> BinaryExpr<LhsT, RhsT const&> const {
    return BinaryExpr<LhsT, RhsT const&>(compareEqual(m_lhs, rhs), m_lhs,
                                         "==", rhs);
  }
  ...
  template <typename RhsT>
  auto operator!=(RhsT const& rhs) -> BinaryExpr<LhsT, RhsT const&> const {
    return BinaryExpr<LhsT, RhsT const&>(compareNotEqual(m_lhs, rhs), m_lhs,
                                         "!=", rhs);
  }
  auto operator!=(bool rhs) -> BinaryExpr<LhsT, bool> const {
    return BinaryExpr<LhsT, bool>(m_lhs != rhs, m_lhs, "!=", rhs);
...
class AssertionHandler {
 ...
 public:
  ...
  void handle(ResultWas::OfType resultType, ITransientExpression const* expr,
              bool negated);
  void handle(AssertionResultData const& resultData,
...
#define INTERNAL_CATCH_REACT(handler) handler.reactWithDebugBreak();
...
MatchAllOf<ComparatorT> MatcherBase<ObjectT, ComparatorT>::operator&&(
    MatcherBase const& other) const {
  return MatchAllOf<ComparatorT>() && *this && other;
}
template <typename ObjectT, typename ComparatorT>
MatchAnyOf<ComparatorT> MatcherBase<ObjectT, ComparatorT>::operator||(
    MatcherBase const& other) const {
...
struct StreamingReporterBase : IStreamingReporter {
  StreamingReporterBase(ReporterConfig const& _config)
    ...
    CATCH_ENFORCE(
        DerivedT::getSupportedVerbosities().count(m_config->verbosity()),
...
AssertionHandler::~AssertionHandler() {
  if (m_inExceptionGuard) {
    handle(ResultWas::ThrewException,
...
void AssertionHandler::handle(AssertionResultData const& resultData,
                              ITransientExpression const* expr) {
...
void AssertionHandler::reactWithDebugBreak() const {
  if (m_shouldDebugBreak) {
    ///////////////////////////////////////////////////////////////////
    // To inspect the state during test, you need to go one level up the
    // callstack To go back to the test and change execution, jump over the
    // reactWithoutDebugBreak() call
    ///////////////////////////////////////////////////////////////////
...
// There is another overload, in catch_assertinhandler.h/.cpp, that only takes a
// string and infers the Equals matcher (so the header does not mention
// matchers)
void handleExceptionMatchExpr(AssertionHandler& handler,
                              StringMatcher const& matcher,
                              StringRef matcherString) {
...
class Context : public IMutableContext, NonCopyable {
 public:  // IContext
  ...
  virtual IConfigPtr getConfig() const override { return m_config; }
...
RunContext::RunContext(IConfigPtr const& _config,
    ...
    : m_runInfo(_config->name()),
      ...
      m_lastAssertionInfo{"", SourceLineInfo("", 0), "",
...
void RunContext::assertionEnded(AssertionResult const& result) {
  ...
  // Reset working state
  m_lastAssertionInfo = {"", m_lastAssertionInfo.lineInfo,
...
void RunContext::assertionRun() { m_prevPassed = m_totals.assertions.passed; }
...
void RunContext::runCurrentTest(std::string& redirectedCout,
  ...
  try {
    m_lastAssertionInfo = {"TEST_CASE", testCaseInfo.lineInfo, "",
                           ResultDisposition::Normal};
    ...
    if (m_shouldReportUnexpected) {
      AssertionHandler(m_lastAssertionInfo.macroName,
...
std::ostream& FileStream::stream() const { return m_ofs; }
...
std::ostream& DebugOutStream::stream() const { return m_os; }
...
StringRef::StringRef(char const* rawChars) noexcept
    : m_start(rawChars), m_size(static_cast<size_type>(std::strlen(rawChars))) {
...
StringRef::StringRef(char const* rawChars, size_type size) noexcept
  ...
  size_type rawSize =
      rawChars == nullptr ? 0 : static_cast<size_type>(std::strlen(rawChars));
  if (rawSize < size) m_size = rawSize;
}

StringRef::StringRef(std::string const& stdString) noexcept
    : m_start(stdString.c_str()), m_size(stdString.size()) {}

StringRef::~StringRef() noexcept { delete[] m_data; }
...
auto StringRef::empty() const noexcept -> bool { return m_size == 0; }

auto StringRef::size() const noexcept -> size_type { return m_size; }
...
AutoReg::AutoReg(ITestInvoker* invoker, SourceLineInfo const& lineInfo,
  ...
  try {
    getMutableRegistryHub().registerTest(
        makeTestCase(invoker, extractClassName(classOrMethod), nameAndTags.name,
                     nameAndTags.tags, lineInfo));
  } catch (...) {
    // Do not throw when constructing global objects, instead register the
    // exception to be processed later
    getMutableRegistryHub().registerStartupException();
...
auto Timer::getElapsedNanoseconds() const -> unsigned int {
  return static_cast<unsigned int>(getCurrentNanosecondsSinceEpoch() -
                                   m_nanoseconds);
}
...
class TablePrinter {
 ...
 public:
  TablePrinter(std::ostream& os, std::vector<ColumnInfo> const& columnInfos)
      : m_os(os), m_columnInfos(columnInfos) {}
...
struct ConsoleReporter : StreamingReporterBase<ConsoleReporter> {
  ...
  void benchmarkStarting(BenchmarkInfo const& info) override {
    ...
    auto nameCol =
        Column(info.name).width(m_tablePrinter.columnInfos()[0].width - 2);
...
class JunitReporter : public CumulativeReporterBase<JunitReporter> {
 public:
  ...
  void testRunEndedCumulative() override { xml.endElement(); }
  ...
  void writeAssertion(AssertionStats const& stats) {
    ...
    if (!result.isOk()) {
      ...
      for (auto const& msg : stats.infoMessages)
        if (msg.type == ResultWas::Info) oss << msg.message << '\n';
...
class XmlReporter : public StreamingReporterBase<XmlReporter> {
 public:
  ...
  virtual std::string getStylesheetRef() const { return std::string(); }
